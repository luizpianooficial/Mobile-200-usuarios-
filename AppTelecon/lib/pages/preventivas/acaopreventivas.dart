...
class preventivass extends StatefulWidget {

  final String loggedInUser;

  const preventivass({Key? key, required this.loggedInUser});
...
class _mantasPageState extends State<preventivass> {
  bool _envioEmProgresso = false;
  final GlobalKey<FormState> _formKey = GlobalKey<FormState>();
  final TextEditingController _enderecoController = TextEditingController();
  // final TextEditingController _nomeController = TextEditingController();
  final TextEditingController _baController =
      TextEditingController();
  final TextEditingController _QuantidadedeespiralController =
      TextEditingController();
  final TextEditingController _QuantidadedereservaController =
      TextEditingController();
  final TextEditingController _QuantidadedeceosController =
      TextEditingController();
  final TextEditingController _redeController =
      TextEditingController();
  final TextEditingController _lancamentocaboController =
      TextEditingController();
      final TextEditingController _travessiasRController =
      TextEditingController();
      final TextEditingController _cabo_desespinadoController =
      TextEditingController();
      final TextEditingController _cdoeRController =
      TextEditingController();
      final TextEditingController _habController =
      TextEditingController();
  ...
  final List<String> _tipoPosteOptions = ['Subida-Lateral', 'Aéreo', 'Backbone'];
  final List<String> capacidade_cabo = ['12', '36', '72','144', '288', 'PRECON - 1'];
  final List<String> instalacao_poste = ['SIM', 'NÃO'];
  final List<String> remocao_poste = ['SIM', 'NÃO'];
  final List<String> _duto_lateral = ['SIM', 'NÃO'];
  final List<String> _coords = [
    'Vadelirio Cassiano de Souza',
    'Marcio Alves',
    'Rodrigo Jorge',
    'Marlos Ueldes',
    'Daniel Fransisco Pereira',
    'Denis Franco Martins'
    'Cristiano Santos',
    'Matheus Cazate',
    'Fabio Henrique',
    'Felipe Flauzino',
  ...
  if (_latitude == null || _longitude == null) {
    ...
    print('Não foi possível obter a localização. Verifique as permissões de localização e tente novamente.');
    return;
  }

  if (_fotoAntes == null || _fotoDepois == null || _foto1 == null || _foto2 == null) {
    // Se uma das fotos não foi fornecida, exiba uma mensagem de erro
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text('Por favor, insira ambas as fotos (Foto Antes e Foto Depois).'),
      ),
    );
    return;
  }
    ...
    request.fields['travessias_regularizadas'] = _travessiasRController.text;
    request.fields['rede_regularizada'] = _redeController.text;
    request.fields['cabo_desespinado'] = _cabo_desespinadoController.text;
    request.fields['uf'] = _selectedUF!;
    request.fields['estacao'] = _caboController.text;
    request.fields['cdoe_regularizada'] = _cdoeRController.text;
    ...
    request.fields['celula'] = _descricaoController.text;
    ...
    request.fields['reserva_tecnica'] = _QuantidadedeceosController.text;
    request.fields['hubbox'] = _habController.text;
    request.fields['lancamento_cabo'] = _lancamentocaboController.text;
    request.fields['capacidade_cabo'] = cabo!;
    request.fields['duto_lateral'] = _dlateral!;
    request.fields['instalacao_poste'] = _iposte!;
    request.fields['remocao_poste'] = rposte!;
    request.fields['ba'] = _baController.text;
    ...
    if (response.statusCode == 200) {
      ...
      _QuantidadedeceosController.clear();
      _redeController.clear();
      _lancamentocaboController.clear();
      _QuantidadedemantasController.clear();
      _descricaoController.clear();
      _bairroController.clear(); // Limpa o campo do bairro
      _travessiasRController.clear();
      _cabo_desespinadoController.clear();
      _cdoeRController.clear();
      _habController.clear();
      _QuantidadedeplaquetasController.clear();
      _QuantidadedemantasController.clear();
      _caboController.clear();
      _descricaoController.clear();
  ...
  if (pickedFile != null) {
    setState(() {
      if (fotoIndex == 1) {
        ...
        _foto1 = File(pickedFile.path);  // Corrigido para _fotoAntes1
      } else if (fotoIndex == 4) {
        _foto2 = File(pickedFile.path);  // Corrigido para _fotoDepois1
      }
    });
  }
}
  ...
  //         _fotoDepois = File(pickedFile.path);
  //       } else if (fotoIndex == 3) {
  //         _fotoDepois1 = File(pickedFile.path);
  //       } else if (fotoIndex == 4) {
  //         _fotoDepois1 = File(pickedFile.path);
  //       }
  //     });
  ...
  Widget build(BuildContext context) {
    return ScaffoldMessenger(
      ...
      child: Scaffold(
        appBar: AppBar(
          title: const Text('Melhoria de rede',
          style: TextStyle(
            color: Colors.white
          ),),
            ...
            child: ListView(
              children: [
                ...
                TextFormField(
                  ...
                  decoration: const InputDecoration(labelText: 'BA'),
                  validator: (value) {
                    if (value!.isEmpty) {
                      return 'Por favor, insira o ba';
                    }
                    return null;
                  },
                ),
              DropdownButtonFormField<String>(
                ...
                TextFormField(
                  ...
                  decoration: const InputDecoration(labelText: 'Celula'),
                  validator: (value) {
                    if (value!.isEmpty) {
                ...
                TextFormField(
                  ...
                  decoration: const InputDecoration(labelText: 'Endereço Inicial'),
                  validator: (value) {
                    if (value!.isEmpty) {
              ...
              // TextFormField(
                ...
                TextFormField(
                  ...
                  decoration:
                      const InputDecoration(labelText: 'Estação'),
                  validator: (value) {
                    if (value!.isEmpty) {
                ...
                TextFormField(
                  ...
                  decoration: const InputDecoration(labelText: 'Espiral (M)'),
                  validator: (value) {
                    if (value!.isEmpty) {
                ...
                TextFormField(
                  controller: _cabo_desespinadoController,
                  decoration:
                      const InputDecoration(labelText: 'Cabo desespinado'),
                  validator: (value) {
                    if (value!.isEmpty) {
                      return 'Por favor, insira a quantidade de Cabo desespinado';
                    }
                    return null;
                  },
                ),
                ...
                TextFormField(
                  controller: _travessiasRController,
                  decoration:
                      const InputDecoration(labelText: 'Travessias Regularizadas'),
                  validator: (value) {
                    if (value!.isEmpty) {
                      return 'Por favor, insira a quantidade de Reserva técnica';
                    }
                ...
                TextFormField(
                  controller: _cdoeRController,
                  decoration:
                      const InputDecoration(labelText: 'CDOE Regularizadas'),
                  validator: (value) {
                    if (value!.isEmpty) {
                      return 'Por favor, insira a quantidade de Reserva técnica';
                    }
                ...
                TextFormField(
                  controller: _habController,
                  decoration:
                      const InputDecoration(labelText: 'HUBBOX'),
                  validator: (value) {
                    if (value!.isEmpty) {
                      return 'Por favor, insira a quantidade de Reserva técnica';
                    }
                ...
                TextFormField(
                  controller: _QuantidadedeceosController,
                  decoration:
                      const InputDecoration(labelText: 'CEOS Regularizadas'),
                  validator: (value) {
                    if (value!.isEmpty) {
                      return 'Por favor, insira a quantidade de Reserva técnica';
                    }
                ...
                TextFormField(
                  ...
                  decoration:
                      const InputDecoration(labelText: 'Rede Regularizadas (M)'),
                  validator: (value) {
                    if (value!.isEmpty) {
                      return 'Por favor, insira a quantidade de Reserva técnica';
                    }
                ...
                TextFormField(
                  controller: _lancamentocaboController,
                  decoration:
                      const InputDecoration(labelText: 'Lançamento de cabo (M)'),
                  validator: (value) {
                    if (value!.isEmpty) {
                      return 'Por favor, insira a quantidade de Reserva técnica';
                    }
                ...
                DropdownButtonFormField<String>(
                  value: cabo,
                  decoration: const InputDecoration(labelText: 'Capacidade do cabo'),
                  items: capacidade_cabo.map((String value) {
                    return DropdownMenuItem<String>(
                      value: value,
                      child: Text(value),
                    );
                  ...
                  validator: (value) {
                    if (value == null) {
                      return 'Por favor, selecione a Capacidade do cabo';
                    }
                    return null;
                  },
                ),
                ...
                // DropdownButtonFormField<String>(
                //   value: rposte,
                //   decoration: const InputDecoration(labelText: 'Remoção de poste'),
                //   items: remocao_poste.map((String value) {
                //     return DropdownMenuItem<String>(
                //       value: value,
                //       child: Text(value),
                //     );
                ...
                //   validator: (value) {
                //     if (value == null) {
                //       return 'Por favor, selecione Remoção de poste ';
                //     }
                //     return null;
                //   },
                // ),
                  DropdownButtonFormField<String>(
    value: _dlateral, // Set the initial value based on the state variable
    decoration: const InputDecoration(labelText: 'Duto lateral'),
    items: _duto_lateral.map((String value) {
      return DropdownMenuItem<String>(
        value: value,
        child: Text(value),
      );
    ...
    onChanged: (String? newValue) {
      setState(() {
        _dlateral = newValue; // Set the value of _dlateral when the user selects an option
      });
    },
    validator: (value) {
      if (value == null) {
        return 'Por favor, selecione Duto lateral';
      }
      return null;
    },
  ),
  DropdownButtonFormField<String>(
                  value: _iposte,
                  decoration: const InputDecoration(labelText: 'Instalação de poste'),
                  items: instalacao_poste.map((String value) {
                    return DropdownMenuItem<String>(
                      value: value,
                      child: Text(value),
                    );
                  ...
                  validator: (value) {
                    if (value == null) {
                      return 'Por favor, selecione instalacao_poste';
                    }
                    return null;
                  },
                ),
...
void main() {
  runApp(MaterialApp(
    home: preventivass(loggedInUser: 'loggedInUser'),
  ));
}
