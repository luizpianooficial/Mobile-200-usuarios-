...
import 'package:moove/pages/ftth/mascara.dart';
...
class _encerramentoState extends State<encerramento> {
  ...
  Future<void> carregarRegistros() async {
    var url = Uri.parse(
        // 'https://sistema32.cloud/move/Api/encerramento.php?user=${widget.loggedInUser}');
        'https://sistema32.cloud/move/Api/VPS/FTTH/encerrar.php?user=${widget.loggedInUser}');
    try {
  ...
  void abrirPixaCaixa(String ba) {
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (context) => MascaraDeEncerramentoPage(ba: ba),
      ),
    );
  }
